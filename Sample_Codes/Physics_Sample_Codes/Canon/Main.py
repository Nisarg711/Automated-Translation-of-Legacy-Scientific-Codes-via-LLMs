...
def calculate(x, y, dt, vinit, theta, Am, nmax, method):
    x[0] = 0.0
    y[0] = 0.0
    ...
    if method == 1:
        ...
        for i in range(1, nmax):
            ...
            vx = vx + (dt/6) * (dvx + 2 * dvx2 + 2 * dvx3 + dvx4)
            vy = vy + (dt/6) * (dvy + 2 * dvy2 + 2 * dvy3 + dvy4)
            if y[i] <= 0.0:
                n = i
                break
    if n == nmax:
...
def main():
    ...
    x = [0.0] * 5003
    y = [0.0] * 5003
    x, y, n = calculate(x, y, dt, vinit, theta, Am, 5003, method)
